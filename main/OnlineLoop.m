...
for i = 1:1:nG
    if isfield(Plant.Generator(i).OpMatB,'Stor')
        stor(i) = i;
        IC(i)=IC(i).*dischEff(stor(i));%scale storage value by discharge efficiency (this scaling is used in optimizations)
...
onlineTime = (Timestamp(A)-DateSim)*24;
onlineTime(end) = TimeDispLoop(1);
[QPall,~] = updateMatrices(QPall,Organize,IC,onlineTime,scaleCost,marginCost,EC);
...
if ~isempty(stor)
    setGen(stor) = (IC(stor)- GenDisp(2,stor)).*(3600/Plant.optimoptions.Topt); %convert change in energy storage to power
end
