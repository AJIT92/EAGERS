...
if (E ==3 || E==4) && isfield(Demand,'E')
    ...
    if ~(strcmp(Plant.Generator(1).Type, 'Utility')&&strcmp(Plant.Generator(1).Source, 'Electricity'))%if you don't already have a utility
    ...
    if ~(strcmp(Plant.Generator(2).Type, 'Utility')&&strcmp(Plant.Generator(2).Source, 'NG'))%if you don't already have a gas utility
...
if E == 4% Test Dynamic Dispatch
    ...
    Plant.optimoptions.Buffer = 0; % percentage for buffer on storage
    Time = buildTimeVector(Plant.optimoptions);
