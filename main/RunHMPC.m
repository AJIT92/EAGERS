function RunHMPC
global Plant  DispatchWaitbar RealTimeData DataLog nLog UB LB% Generators Interval Resolution Horizon Tmpc Topt t_mpc spGEN  %passed from GUI
global RealTime Virtual scaleTime OptimalMap MapWaitbarHandle DateSim NumSteps%relates to how optimization proceeds
global FanPortWrite %related to communication for Savona E-Hub
global CurrentState Dispatch Operation Si Last24hour OnOff t_mpc%initialized here
...
%DispatchWaitbar: handle to the dialog box showing the progress
...
Xf = nnz(Plant.Data.Timestamp<=DateSim+options.Horizon/24);
...
if RealTime
    ...
    K =1;
else K = menu('Select Option','Manually specify initial conditions','Automatically determine initial conditions');
end
%% Step 2 Initialize Variables & prepare to iterate time steps
...
Dispatch.RunData.Timestamp = zeros((options.Horizon)*nMPC+1,1);
Dispatch.RunData.GeneratorState = zeros((options.Horizon)*nMPC+1,length(UB));
...
for i = 1:1:length(S)
    ...
    Dispatch.RunData.Demand.(S{i}) = zeros((options.Horizon)*nMPC+1,1);
...
DispatchWaitbar=waitbar(0,'Running Dispatch');
