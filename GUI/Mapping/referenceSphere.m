...
% $Revision: 1.1.6.5 $  $Date: 2012/11/02 14:21:16 $
    ...
    methods
        ...
        function disp(self)
            % Override the default in order to display only the Name,
            % LengthUnit, and Radius properties.
            
            usingDesktop = usejava('desktop') && desktop('-inuse') ...
                && feature('hotlinks');
            
            if usingDesktop
                lnk = '<a href="matlab:help referenceSphere">referenceSphere</a>';
            else
                lnk = 'referenceSphere';
            end
            
            s = struct('Name',self.Name,'LengthUnit',self.pLengthUnit,'Radius',self.a); %#ok<NASGU>
            fprintf('  %s\n\n  Properties:\n%s', ...
                lnk, evalc('builtin(''disp'',s)'))
        end
