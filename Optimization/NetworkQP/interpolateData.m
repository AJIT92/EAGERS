...
elseif r>1 %interpolate between timesteps
 ...
 %Modify this to create two arrays, not just one
    for j = 1:1:length(S)
        for v = 1:1:length(TestData.Demand);
            interpData.Demand(v).(S{j}) = zeros(NumSteps,1); 
        end
    end
    nn = length(Noise);
    for j = 1:1:length(S)
        for v = 1:1:length(TestData.Demand)
            ...
            TestDemandcell = TestData.Demand(v).(S{j});%   %modified to allow for multiple demands
            prev = TestDemandcell(x1);            
            x2 = x1+1;
            next = TestDemandcell(x2);            
            while i<NumSteps
                i=i+1;
                ni = mod(xi+i-1,nn)+1;
                r0 = (mod(i-1,r))/r;
                ...
                if TestData.Timestamp(x2)<=interpData.Timestamp(i)
                    ...
                    TestDemandcell= TestData.Demand(v).(S{j});
                    next = TestDemandcell(x2);
                end
                interpData.Demand(v).(S{j})(i) = ((1-r0)*prev+r0*next)*(1+Noise(ni));
            end 
        end  
    end  
end 