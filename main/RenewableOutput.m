    ...
    if ismember(cellstr(Plant.Generator(i).Type),include) && Plant.Generator(i).Enabled == 1 
        ...
        elseif strcmp(str,'Predict')
            ...
            if strcmp(Gen.Type,'Solar') 
                ...
                day1= Power(Xts:XFts);
                day2 = Power(min(n-(XFts-Xts),Xts+1*96):min(n,XFts+2*96));
                day3 = Power(min(n-(XFts-Xts),Xts+2*96):min(n,XFts+3*96));
                day4 = Power(min(n-(XFts-Xts),Xts+3*96):min(n,XFts+4*96));
