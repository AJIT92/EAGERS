...
Demandsum = 0; %Combine demand for energy balance?

for i = 1:1:length(S)
    for v = 1:1:length(RealData.Demand)
    Operation.Demand(v).(S{i})(t_mpc) = RealData.Demand(v).(S{i});
    Demandsum = Demandsum + RealData.Demand(v).(S{i}); 
    end
end
...
%         DemandH = RealData.Demand.H; %Currently no heat
DemandH = [];
    else DemandH = [];
    end
    ...
    if RealTime %get measured outputs (current state)
        ...
        %% --- Determine grid & energy storage balance
        Demand = Demand - sum(CurrentState.Generators(thisSeq));
        Operation.GeneratorState(t_mpc,[stor,utility]) = GRIDandStorage(Demand,setGen,stor,utility);
        if ~isempty(DemandC)
            DemandC = DemandC - sum(CurrentState.Generators(chill));
            Operation.GeneratorState(t_mpc,[storC,utilC]) = GRIDandStorage(DemandC,setGen,storC,utilC);
