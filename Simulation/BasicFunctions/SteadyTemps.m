function [T,HTcond,HTconv,HTrad]= SteadyTemps(block,Flow1,Flow2)
...
q1 = SpecHeat(Flow1).*NetFlow(Flow1)/(C*rows);
s = 2; %at minimum a flow and a solid mass (reformer)
inT = 1;
if ~isempty(Flow2)
    if strcmp(block.direction ,'crossflow')
        q2 = SpecHeat(Flow2).*NetFlow(Flow2)/(C*columns);
    else
        q2 = SpecHeat(Flow2).*NetFlow(Flow2)/(C*rows);
    end
...
if isfield(block,'FCtype') %fuel cell or electrolyzer
    ...
    h_rxn4 = h.CO+.5*h.O2-h.CO2;
    Q.gen = (block.Current.H2/(2*F).*h_rxn3 + block.Current.CO/(2*F).*h_rxn4 - (block.Current.H2+block.Current.CO)*block.Voltage)/1000;%kW of heat generated by electrochemistry (per node & per cell)
